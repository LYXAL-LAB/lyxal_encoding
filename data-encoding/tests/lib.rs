...
mod property_tests {
	...
	proptest! {
		...
		fn base64_roundtrip(data in any::<Vec<u8>>()) {
			...
			let decoded = BASE64.decode(encoded.as_bytes()).expect("BASE64 decoding failed");
			prop_assert_eq!(data, decoded);
		}

		#[test]
		fn base32_roundtrip(data in any::<Vec<u8>>()) {
			let encoded = BASE32.encode(&data);
			let decoded = BASE32.decode(encoded.as_bytes()).expect("BASE32 decoding failed");
			prop_assert_eq!(data, decoded);
		}

		#[test]
		fn hex_roundtrip(data in any::<Vec<u8>>()) {
			let encoded = HEXLOWER.encode(&data);
			let decoded = HEXLOWER.decode(encoded.as_bytes()).expect("HEXLOWER decoding failed");
			prop_assert_eq!(data, decoded);
		}
	}
}
...
fn base64_error() {
	...
	assert_eq!(
		b.decode(b"YmE=-mFyZg==").err().unwrap(),
		DecodeError {
			position: 4,
			kind: Symbol
		}
	...
	assert_eq!(
		b.decode(b"YmE=-g==Zg==").err().unwrap(),
		DecodeError {
			position: 4,
			kind: Symbol
		}
	...
	assert_eq!(
		b.decode(b"YmE=Y-FyZg==").err().unwrap(),
		DecodeError {
			position: 5,
			kind: Symbol
		}
